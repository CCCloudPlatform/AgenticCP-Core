...
public class InfluxDBStorage implements MetricsStorage {
    ...
    public void saveMetrics(List<Metric> metrics) {
        if (!enabled) {
            log.debug("[InfluxDBStorage] saveMetrics - InfluxDB 저장소가 비활성화되어 있습니다. 메트릭 저장을 건너뜁니다.");
            return;
        }
        
        if (!isConnected()) {
            log.warn("[InfluxDBStorage] saveMetrics - InfluxDB에 연결되지 않았습니다. 연결을 시도합니다.");
            connect();
        }
        
        try {
            log.debug("[InfluxDBStorage] saveMetrics - InfluxDB에 메트릭 저장 시작: {} 개", metrics.size());
            
            // TODO: 실제 InfluxDB 클라이언트를 사용한 저장 로직 구현
            // 현재는 로그만 출력
            for (Metric metric : metrics) {
                log.debug("[InfluxDBStorage] saveMetrics - 메트릭 저장: name={}, value={}, timestamp={}", 
                    metric.getMetricName(), metric.getMetricValue(), metric.getCollectedAt());
            }
            
            log.debug("[InfluxDBStorage] saveMetrics - InfluxDB 메트릭 저장 완료: {} 개", metrics.size());
            
        } catch (Exception e) {
            log.error("[InfluxDBStorage] saveMetrics - InfluxDB 메트릭 저장 실패", e);
    ...
    public List<Metric> getMetrics(String metricName, LocalDateTime startTime, LocalDateTime endTime) {
        if (!enabled) {
            log.debug("[InfluxDBStorage] getMetrics - InfluxDB 저장소가 비활성화되어 있습니다.");
            return List.of();
        }
        
        if (!isConnected()) {
            log.warn("[InfluxDBStorage] getMetrics - InfluxDB에 연결되지 않았습니다. 연결을 시도합니다.");
            connect();
        }
        
        try {
            log.debug("[InfluxDBStorage] getMetrics - InfluxDB에서 메트릭 조회: name={}, start={}, end={}", 
            ...
            log.debug("[InfluxDBStorage] getMetrics - InfluxDB 메트릭 조회 완료: {} 개", metrics.size());
            return metrics;
            
        } catch (Exception e) {
            log.error("[InfluxDBStorage] getMetrics - InfluxDB 메트릭 조회 실패", e);
    ...
    public void connect() {
        try {
            log.info("[InfluxDBStorage] connect - InfluxDB 연결 시도: url={}, database={}", config.getUrl(), config.getDatabase());
            
            // TODO: 실제 InfluxDB 클라이언트를 사용한 연결 로직 구현
            // 현재는 연결 성공으로 가정
            connected = true;
            
            log.info("[InfluxDBStorage] connect - InfluxDB 연결 성공");
            
        } catch (Exception e) {
            log.error("[InfluxDBStorage] connect - InfluxDB 연결 실패", e);
            connected = false;
    ...
    public void disconnect() {
        try {
            log.info("[InfluxDBStorage] disconnect - InfluxDB 연결 해제");
            
            // TODO: 실제 InfluxDB 클라이언트를 사용한 연결 해제 로직 구현
            connected = false;
            
            log.info("[InfluxDBStorage] disconnect - InfluxDB 연결 해제 완료");
            
        } catch (Exception e) {
            log.error("[InfluxDBStorage] disconnect - InfluxDB 연결 해제 실패", e);
        }
    }
    ...
    public void setEnabled(boolean enabled) {
        log.info("[InfluxDBStorage] setEnabled - InfluxDB 저장소 활성화 상태 변경: {}", enabled);
