...
public enum TargetingRuleErrorCode implements BaseErrorCode {
    ...
    // 클라우드 환경 관련 에러
    CLOUD_ENVIRONMENT_INVALID(HttpStatus.BAD_REQUEST, 6137,
            "유효하지 않은 클라우드 환경 타입입니다."),
    CLOUD_ENVIRONMENT_DETECTION_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, 6138,
            "클라우드 환경 감지 중 오류가 발생했습니다."),
    CLOUD_RESOURCE_NOT_FOUND(HttpStatus.NOT_FOUND, 6139,
            "테넌트의 클라우드 리소스를 찾을 수 없습니다."),
    TENANT_ID_REQUIRED_FOR_ENVIRONMENT_RULE(HttpStatus.BAD_REQUEST, 6140,
            "클라우드 환경 규칙 평가를 위해서는 테넌트 ID가 필수입니다."),
    RULE_VALUE_EMPTY_ARRAY(HttpStatus.BAD_REQUEST, 6141,
            "규칙 값 배열이 비어있습니다. 최소 하나 이상의 값이 필요합니다."),
    CLOUD_PROVIDER_MISSING(HttpStatus.BAD_REQUEST, 6142,
            "클라우드 리소스에 프로바이더 정보가 없습니다.");
